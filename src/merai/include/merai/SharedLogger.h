...
namespace merai
{
    ...
    {
        std::uint64_t       timestamp;      // nanoseconds since steady_clock epoch
        shared_log_level    level;
        int                 code;
        std::uint32_t       text_length = 0; // bytes valid in text
    ...
    struct logger_shared_memory
    {
        ...
        std::atomic<std::size_t>    head{0};      // producer
        std::atomic<std::size_t>    tail{0};      // consumer
        std::atomic<std::uint64_t>  dropped{0};   // number of messages overwritten
    };

    inline void push_log_message(logger_shared_memory* shm,
                                 const shared_log_message& msg)
    {
        std::size_t head      = shm->head.load(std::memory_order_relaxed);
        std::size_t next_head = (head + 1) % log_buffer_size;
        ...
        // If buffer was full, move tail and count drop
        if (next_head == shm->tail.load(std::memory_order_acquire))
        {
            std::size_t old_tail =
                (shm->tail.load(std::memory_order_relaxed) + 1) % log_buffer_size;
            shm->tail.store(old_tail, std::memory_order_release);
            shm->dropped.fetch_add(1, std::memory_order_relaxed);
        }
    }

    inline bool pop_log_message(logger_shared_memory* shm,
                                shared_log_message& out_msg)
    {
        std::size_t tail = shm->tail.load(std::memory_order_relaxed);
        std::size_t head = shm->head.load(std::memory_order_acquire);
        ...
        std::size_t next_tail = (tail + 1) % log_buffer_size;
    ...
    {
        static constexpr std::uint32_t MAGIC   = 0x4C4F4747; // 'LOGG'
    ...
    inline void push_fieldbus_log(multi_ring_logger_memory* multi,
                                  const shared_log_message& msg)
    {
        push_log_message(&multi->fieldbus_ring, msg);
    }

    inline bool pop_fieldbus_log(multi_ring_logger_memory* multi,
                                 shared_log_message& out_msg)
    {
        return pop_log_message(&multi->fieldbus_ring, out_msg);
    }

    inline void push_control_log(multi_ring_logger_memory* multi,
                                 const shared_log_message& msg)
    {
        push_log_message(&multi->control_ring, msg);
    }

    inline bool pop_control_log(multi_ring_logger_memory* multi,
                                shared_log_message& out_msg)
    {
        return pop_log_message(&multi->control_ring, out_msg);
    }

    inline void push_logic_log(multi_ring_logger_memory* multi,
                               const shared_log_message& msg)
    {
        push_log_message(&multi->logic_ring, msg);
    }

    inline bool pop_logic_log(multi_ring_logger_memory* multi,
                              shared_log_message& out_msg)
    {
                            ...
                            const char* text)
    {
        ...
        msg.text_length = static_cast<std::uint32_t>(std::strlen(msg.text));

        push_log_message_for_module(multi, module_name, msg);
    }

    // Convenience wrappers per log level
