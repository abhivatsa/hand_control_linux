...
namespace control
{
    ...
    bool HomingController::start(std::span<const merai::JointMotionFeedback> motionFbk,
                                 std::span<merai::JointMotionCommand> motionCmd)
    {
        ...
        for (int i = 0; i < numJoints_ && i < static_cast<int>(motionCmd.size()); ++i)
        {
            motionCmd[static_cast<std::size_t>(i)].modeOfOperation = 8;
        }
        ...
        for (int i = 0; i < numJoints_; i++)
        {
            ...
            currentPos[i] = motionFbk[static_cast<std::size_t>(i)].positionActual;
        }
        planTrajectory(currentPos);
        return true;
    }

    void HomingController::update(std::span<const merai::JointMotionFeedback> motionFbk,
                                  std::span<merai::JointMotionCommand> motionCmd,
                                  double dt)
    {
        ...
        if (isActive_)
        {
            ...
            for (int j = 0; j < numJoints_; j++)
            {
                ...
                if (j < static_cast<int>(motionCmd.size()))
                {
                    motionCmd[static_cast<std::size_t>(j)].targetPosition = pos;
                }
                ...
                if (j < static_cast<int>(motionCmd.size()))
                {
                    motionCmd[static_cast<std::size_t>(j)].targetTorque = 0.0;
                }

                // overriding the values
                if (j == 5 && j < static_cast<int>(motionCmd.size()))
                {
                    motionCmd[static_cast<std::size_t>(j)].targetPosition = motionFbk[static_cast<std::size_t>(j)].positionActual;
                }

                if (j == 0 && j < static_cast<int>(motionCmd.size()))
                {
                    motionCmd[static_cast<std::size_t>(j)].targetPosition = 4.3488;
                }
        ...
        else
        {
            ...
            holdPosition(motionFbk, motionCmd);
        }
    }
    ...
    void HomingController::holdPosition(std::span<const merai::JointMotionFeedback> motionFbk,
                                        std::span<merai::JointMotionCommand> motionCmd)
    {
        ...
        for (int j = 0; j < numJoints_; j++)
        {
            ...
            double currentPos = motionFbk[static_cast<std::size_t>(j)].positionActual;
            motionCmd[static_cast<std::size_t>(j)].targetPosition = currentPos;
