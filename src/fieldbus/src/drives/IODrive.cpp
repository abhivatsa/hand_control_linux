...
namespace fieldbus
{
    ...
    bool IoDrive::configurePdos()
    {
        if (driveCfg_.syncManagerCount == 0)
        {
            if (loggerMem_)
            {
                merai::log_error(loggerMem_, "IoDrive", 110, "No sync managers found in DriveConfig");
            }
            return false;
        }
        ...
        for (int smIndex = 0; smIndex < driveCfg_.syncManagerCount; ++smIndex)
        {
            ...
            int smId = sm.id;
            ec_direction_t direction = (sm.type == merai::SyncType::RxPdo) ? EC_DIR_OUTPUT : EC_DIR_INPUT;
            ...
            for (int assignIdx = 0; assignIdx < sm.assignmentCount; ++assignIdx)
            {
                ...
                if (ecrt_slave_config_pdo_assign_add(slaveConfig_, smId, assignmentAddr))
                {
                    if (loggerMem_)
                    {
                        merai::log_error(loggerMem_, "IoDrive", 112, "Failed to add PDO assignment to SM");
                    }
                    return false;
                }
            ...
            for (int mgIndex = 0; mgIndex < sm.mappingGroupCount; ++mgIndex)
            {
                ...
                for (int eIndex = 0; eIndex < mg.entryCount; ++eIndex)
                {
                            ...
                            bitLen))
                    {
                        if (loggerMem_)
                        {
                            merai::log_error(loggerMem_, "IoDrive", 113,
        ...
        if (!registerPdoEntries())
        {
            if (loggerMem_)
            {
                merai::log_error(loggerMem_, "IoDrive", 114, "Failed to register PDO entries for IoDrive");
            }
            return false;
        }
    ...
    bool IoDrive::registerPdoEntries()
    {
            ...
            for (int mgIndex = 0; mgIndex < sm.mappingGroupCount; mgIndex++)
            {
                ...
                for (int eIndex = 0; eIndex < mg.entryCount; eIndex++)
                {
                    ...
                    if (idx >= MAX_ENTRIES)
                    {
                        if (loggerMem_)
                        {
                            merai::log_warn(loggerMem_, "IoDrive", 117, "Too many PDO entries. Increase MAX_ENTRIES.");
                        }
                        return false;
                    }
                    ...
                    if (!offsetPtr)
                    {
                        if (loggerMem_)
                        {
                            merai::log_error(loggerMem_, "IoDrive", 118, "Unsupported object_index in PDO mapping");
                        }
                        return false;
                    }
        ...
        if (ecrt_domain_reg_pdo_entry_list(domain_, domainRegs))
        {
            if (loggerMem_)
            {
                merai::log_error(loggerMem_, "IoDrive", 119, "PDO entry registration failed");
            }
            return false;
        }
        return true;
