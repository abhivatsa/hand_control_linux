...
namespace fieldbus
{
    IoDrive::IoDrive(const merai::DriveConfig&    driveCfg,
    ...
    void IoDrive::initialize()
    {
        ...
        if (loggerMem_)
        {
            merai::log_info(loggerMem_, "IoDrive", 101, "IoDrive initialized");
        }
    }
    ...
    bool IoDrive::configurePdos()
    {
        ...
        for (int smIndex = 0; smIndex < driveCfg_.syncManagerCount; ++smIndex)
        {
            ...
            if (ecrt_slave_config_sync_manager(slaveConfig_, smId, direction, wdMode))
            {
                if (loggerMem_)
                {
                    merai::log_error(loggerMem_, "IoDrive", 111, "Failed to configure SM");
                }
                return false;
            }
            ...
            if (ecrt_slave_config_pdo_assign_clear(slaveConfig_, smId))
            {
                if (loggerMem_)
                {
                    merai::log_error(loggerMem_, "IoDrive", 120, "Failed to clear PDO assignments");
                }
                return false;
            }
            ...
            {
                const std::uint16_t assignmentAddr =
            ...
            for (int mgIndex = 0; mgIndex < sm.mappingGroupCount; ++mgIndex)
            {
                ...
                const std::uint16_t pdoAddress = static_cast<std::uint16_t>(mg.assignmentKey);
        ...
        // Register PDO entries (currently stubbed; will fail due to null offsets)
        if (!registerPdoEntries())
        {
            if (loggerMem_)
            {
                merai::log_error(loggerMem_,
                                 "IoDrive",
                                 114,
                                 "Failed to register PDO entries for IoDrive");
            }
            return false;
        }
        ...
        if (loggerMem_)
        {
            merai::log_info(loggerMem_, "IoDrive", 115, "IoDrive PDOs configured successfully");
        }
        return true;
    }
    ...
    bool IoDrive::registerPdoEntries()
    {
        ...
        const std::uint16_t alias       = driveCfg_.alias;
        const std::uint16_t position    = driveCfg_.position;
        const std::uint32_t vendorId    = driveCfg_.vendor_id;
        const std::uint32_t productCode = driveCfg_.product_code;
        ...
        for (int smIndex = 0; smIndex < driveCfg_.syncManagerCount; ++smIndex)
        {
            ...
            for (int mgIndex = 0; mgIndex < sm.mappingGroupCount; ++mgIndex)
            {
                ...
                (void)pdoAddress; // not used directly here, but part of config above
                ...
                for (int eIndex = 0; eIndex < mg.entryCount; ++eIndex)
                {
                    ...
                    const auto&     pme      = mg.entries[eIndex];
                    std::uint16_t   objIndex = static_cast<std::uint16_t>(pme.object_index);
                    std::uint8_t    subIndex = static_cast<std::uint8_t>(pme.subindex);
                    ...
                    if (!offsetPtr)
                    {
                        if (loggerMem_)
                        {
                            merai::log_error(loggerMem_,
                                             "IoDrive",
                                             118,
                                             "Unsupported object_index in PDO mapping");
                        }
                        return false;
                    }
                    ...
                    reg.bit_position = 0; // byte-aligned
                }
            }
        }
    ...
    bool IoDrive::readInputs(std::uint8_t* domainPd)
    {
        if (!domainPd)
        {
            return false;
        ...
        // Stub: assume digital input mapped at ioOffsets_.inputOffset
        digitalInputs_ = EC_READ_U16(domainPd + ioOffsets_.inputOffset);
        return true;
    }

    bool IoDrive::writeOutputs(std::uint8_t* domainPd)
    {
        if (!domainPd)
        {
            return false;
        ...
        // Stub: assume digital output mapped at ioOffsets_.outputOffset
    ...
    {
        // No advanced logic for simple I/O yet
    }

    void IoDrive::setDigitalOutput(std::uint16_t output)
    {
        digitalOutputs_ = output;
        if (loggerMem_)
        {
            merai::log_debug(loggerMem_, "IoDrive", 130, "Digital output updated");
        }
    }

    std::uint16_t IoDrive::getDigitalInput() const
    {
        return digitalInputs_;
    }

    void* IoDrive::getOffsetPointerByIndex(std::uint16_t /*index*/, std::uint8_t /*subIndex*/)
    {
        // NOTE:
        //  This is intentionally left as a stub. Once you decide on the object indexes
        //  for IO inputs/outputs (e.g., 0x6000 / 0x7000 etc.), implement the mapping:
        //
        //  switch (index) {
        //    case 0x6000: return &ioOffsets_.inputOffset;
        //    case 0x7000: return &ioOffsets_.outputOffset;
        //    ...
        //  }
        //
        // For now, any IO drive in the config will make configurePdos() fail.
        return nullptr;
    }
