...
namespace hand_control
{
    namespace network_api
    {
        ...
        const hand_control::merai::JointFeedbackData *HapticDeviceAPI::getCurrentFeedback() const
        {
            ...
            const auto &feedbackArray = rtLayout_->jointBuffer.buffer[frontIdx].feedback.data();
        ...
        bool HapticDeviceAPI::getPosition(double &x, double &y, double &z) const
        {
            ...
            x = fb[0].motion.positionActual; // might need scaling
            y = fb[1].motion.positionActual; // might need scaling
            z = fb[2].motion.positionActual; // might need scaling
