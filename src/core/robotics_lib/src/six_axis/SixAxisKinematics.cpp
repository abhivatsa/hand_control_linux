...
namespace motion_control
{
    namespace robotics
    {
        namespace six_axis
        {
            ...
            SixAxisKinematics::SixAxisKinematics(const SixAxisModel &model)
                : model_(model)
            {
                // If you want to precompute anything from model_, do it here.
            }

            int SixAxisKinematics::forwardKinematics(
                const motion_control::math::Vector<6> &jointAngles,
            ...
            int SixAxisKinematics::computeJacobian(
                const motion_control::math::Vector<6> &jointAngles,
                motion_control::math::Matrix<6, 6> &outJacobian) const
            {
                ...
                for (int i = 0; i < SixAxisModel::NUM_JOINTS; ++i)
                {
                    ...
                    // The joint axis in world coords (assuming Z axis = rotation axis):
            ...
            int SixAxisKinematics::computeInverseKinematics(
                const motion_control::math::Vector<6> &currentJointAngles,
            ...
            motion_control::math::Matrix<4, 4> SixAxisKinematics::buildHomogeneousTransform(
            ...
            motion_control::math::Matrix<3, 3> SixAxisKinematics::axisAngleRotation(
